...
Remove-Item -Path "$toolsDir\ndk.zip" -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$toolsDir\cmake.zip" -Force -ErrorAction SilentlyContinue
...
Write-Host "Downloading Android NDK (r26b) using curl..."
curl.exe -L -o $ndkZip $ndkUrl

Write-Host "Downloading CMake (3.29.3) using curl..."
